...
{                         // Struct of args for each thread
    ...
    char server_msg[128]; // Server message
    int tid;              // Thread id
};
...
int main()
{
    ...
    int client_count = -1;             // Set the client_count
    pthread_t threads[MAXTHREADCOUNT]; // Create a pool of threads
    struct args thread_args;

    while (1)
    { // Thread initilization loop, creates and detaches threads for each Client connection
        if (client_count == MAXTHREADCOUNT)
        {
